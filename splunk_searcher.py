...
import json, sys, re, logging, signal, csv, getpass, argparse, gzip, pickle, click

from time import sleep
from datetime import datetime, timedelta, time
from os.path import join, isfile, isdir, exists, abspath, dirname
...
# once = attempt resume-able exit (finish running chunks if chunked search or pause job, save progress)
# twice = attempt clean exit (finalize running jobs instead of waiting for them to finish in chunked search)
...
def handle_sigint(signal: int, frame: Optional[FrameType]) -> None:
    ...
    if QUIT_C >= 3:
        loggy.error("Ctrl+c pressed 3 times, force quitting. This may leave jobs hanging or results un-exported and will not allow for resume.")
        quit()
    elif QUIT_C >= 2:
        loggy.warning("Press ctrl+c again to force quit (WARNING: this may leave jobs running on the searchhead and cannot be resumed)")
...
def print_search_options(options: SearchOptions) -> None:
    if options["chunk_options"]:
        loggy.info(f"Chunked Search")
        if options["export_threads"]:
            loggy.info("Multithreaded export enabled with {} threads.".format(options["export_threads"]))
    else:
        loggy.info("Single Job Search")
    loggy.info("Query: '{}'".format(options["search"]))
    loggy.info("Start Date: {}".format(options["start_date"]))
    loggy.info("End Date: {}".format(options["end_date"]))
    if options["export_options"]["export_path"]:
        loggy.info("Export path: {}".format(abspath(options["export_options"]["export_path"])))
    if options["export_options"]["merge_path"]:
        loggy.info("Merge path: {}".format(abspath(options["export_options"]["merge_path"])))
    if options["chunk_options"]:
        loggy.info("Chunk size: {}".format(options["chunk_options"]["chunk_size"]))
        loggy.info("Start at search: {}".format(options["chunk_options"]["start_at"]))
    loggy.info("Progress path: {}".format(options["progress_path"]))

    if options["job_limits"]:
        loggy.info("Using {} jobs concurrently when not limited.".format(options["job_limits"]["default_n_jobs"]))
        for limit_ in options["job_limits"]["limits"]:
            loggy.info("Limit of {} jobs between {} and {}".format(limit_["jobs"],limit_["start"],limit_["stop"]))


def validate_export_options(options: ExportOptions) -> bool:
    valid = True
    if not options["export_path"] and not options["merge_path"]:
        loggy.error("No export_path or merge_path defined, results would not be exported!")
        valid = False
    ...
    if options["merge_path"]:
        ...
        if not exists(merge_dir):
            loggy.warning(f"Path to merge file {merge_dir} does not appear to exist, it will be created.")
        
        if exists(options["merge_path"]):
            if isdir(options["merge_path"]):
                loggy.error("merge_path {} already exists and is not a file".format(options["merge_path"]))
                valid = False
            else:
    ...
    if options["export_mode"] == "dump":
        if not exists(get_splunk_dispatch_path()):
            loggy.error("Using 'dump' export-mode but Splunk dispatch folder could not be found at {}".format(get_splunk_dispatch_path()))
            loggy.error("Script must run locally on search head and have permission to read dispatch directory when using 'dump' export-mode")
            valid = False
    
    if options["export_path"]:
        if exists(options["export_path"]):
            if not isdir(options["export_path"]):
                loggy.error("Export path {} is not a directory.".format(options["export_path"]))

            # list files at directory name of absolute path of export path
            if len(listdir(dirname(abspath(options["export_path"])))) > 0:
                loggy.warning("Export path {} is not empty".format(options["export_path"]))
        else:
            loggy.warning("Export path {} does not exist, all directories will be created".format(options["export_path"]))
...
def validate_job_limits(job_limits: list[JobLimit]) -> bool:
    ...
    for limit_idx in range(len(job_limits)):
        ...
        try:
            ...
            loggy.error(f"Error: could not parse job limits start or stop time, {job_limits[limit_idx]}")
            return False

        # make sure stop time is after start time
        if stop_time <= start_time:
            loggy.error(f"Error: job limit stop time is before or equal to start time, {job_limits[limit_idx]}")
            return False
        ...
        if limit_idx < len(job_limits) - 1:
            ...
            if stop_time > next_start:
                loggy.error(f"Error: job limit stop time overlaps with other limit start time, {job_limits[limit_idx]} and {job_limits[limit_idx+1]}")
                return False
    return True
...
def validate_search_options(options: SearchOptions) -> None:
    ...
    if options["export_options"]["export_mode"] == "dump":
        if "|dump" not in options["search"] and "| dump" not in options["search"]:
            loggy.error("Using 'dump' export-mode but search string does not appear to use the dump command")
            valid = False
    else:
        if "|dump" in options["search"] or "| dump" in options["search"]:
            loggy.warning("Using |dump Splunk command but export mode is {}".format(options["export_options"]["export_mode"]))
            loggy.info("If you want to set the export format of the dump results, use format option like '| dump basefilename=<filename> format=[raw | csv | tsv | json | xml]'")
...
def create_search_job(splunk_client: client.Service, search_str: str) -> client.Job:
    ...
    loggy.debug(f"Created search job, SID: {job.name} Search: '{search_str}'")
    return job
...
def setup_export(options: ExportOptions) -> None:
    if options["export_path"] and not exists(options["export_path"]):
        loggy.info("Could not find export path {}, creating dir(s)".format(options["export_path"]))
...
def export_job(job: client.Job, options: ExportOptions) -> Optional[int]:
    ...
    if options["export_mode"] == "dump":  # use local dump export mode
        ...
        if not exists(splunk_dispatch_path):
            loggy.error(f'Splunk dispatch folder {splunk_dispatch_path} does not seem to exist but output_mode is "dump". Script must run local to search head if using dump mode.')
        ...
        if not exists(dump_folder):
            loggy.warning(f"Dump folder not found for SID {job.name}")
            return None
        dump_files = listdir(dump_folder)

        if not dump_files:
            loggy.debug(f"No dump files found for {job.name}.")
            job.set_ttl(1)
            return 0

        # open gzip files and append to the merge path
        if options["merge_path"]: # read and merge dump files into merge path
            loggy.debug("Merging {} dump files to {}".format(len(dump_files), join(options["merge_path"])))
        ...
        if options["export_path"]:
            ...
            mkdir(dest_folder)
            loggy.debug(f"Moving {len(dump_files)} dump files from {dump_folder} to {dest_folder}")
        ...
        if options["export_path"]: # write results to a file with sid as name in export path
            with open(join(options["export_path"], f"{job.name}"), "w") as single_result:
...
# ------------------ Chunked search --------------------------
def chunk_range(start_date: datetime, end_date: datetime, chunk_size: timedelta) -> list[tuple[datetime, datetime]]:
...
def chunk_search(options: SearchOptions) -> list[str]:
    ...
    loggy.info("Chunking search {} from {} to {} with chunk_size of {}".format(options["search"],range_start_str, range_end_str, options["chunk_options"]["chunk_size"]))
    # get datetime chunks
    date_chunks = chunk_range(options["start_date"], options["end_date"], options["chunk_options"]["chunk_size"])
    searches = []
    for chunk in date_chunks:
        ...
        new_search = options["search"].format(earliest=start_date_str, latest=end_date_str)
...
def get_max_jobs(job_limits: JobLimits) -> int:
    ...
    job_limits["limits"] = sorted(job_limits["limits"], key=lambda x: str_to_time(x["start"]))
...
def remove_done_exports(export_jobs: list[Future[Optional[int]]]) -> list[Optional[int]]:
    finished = []
...
def multi_job_search(splunk_client: client.Service, search_options: SearchOptions, searches: list[str]) -> None:
    ...
    # list of completed sids, this allows merge function to avoid existing data in export_path
    ...
    if search_options["export_threads"]:
        ...
        export_threads: ThreadPoolExecutor = ThreadPoolExecutor(search_options["export_threads"])
        # pending export jobs
    ...
    while (jobs_remain() or (searches_remain() and not quitting)) and not QUIT_C > 1:
        ...
        if len(finished_jobs) > 0:
            ...
            for job in finished_jobs:
                if search_options["export_threads"]:
                    export_jobs.append(export_threads.submit(export_job, job, search_options["export_options"]))
                else:
                    # export the job results and get count if any were exported
                    job_results_count = export_job(job, search_options["export_options"])
                    if job_results_count:
                        cycle_result_count += job_results_count
                        total_results += job_results_count
            if cycle_result_count > 0:
                loggy.info(f"{len(finished_jobs)} jobs finished with {cycle_result_count} bytes exported.")
            elif not search_options["export_threads"]:
                loggy.info(f"{len(finished_jobs)} jobs finished with no results.")
            else:
                loggy.info(f"{len(finished_jobs)} search jobs finished, added to export queue.")
        ...
        if search_options["export_threads"]:
            ...
            if num_exports_completed > 0:
                ...
                num_results_exported = sum([result for result in completed_export_jobs if result])
                total_results += num_results_exported
                if num_results_exported > 0:
                    loggy.info(f"{num_exports_completed} export jobs finished, {num_results_exported} bytes exported.")
                else:
                    loggy.info(f"{num_exports_completed} export jobs finished with no results.")

        if QUIT and not quitting: 
            loggy.info("Attempting safe quit, finishing running/paused jobs and saving progress.")
            quitting=True
        ...
        if to_add > 0:
            ...
            for _ in range(to_add):
                ...
                if len(paused_jobs) > 0:
                    ...
                    running_jobs.append(create_search_job(splunk_client, searches[searches_idx]))
                    searches_idx += 1
        elif to_add < 0: # pause jobs since there are too many
            loggy.debug(f"Pausing {-to_add} jobs.")
            for job_idx in range(-to_add):
        ...
        if search_options["export_threads"]:
            loggy.info(f"{len(running_jobs)} / {target_amt_jobs} jobs running, {len(paused_jobs)} jobs paused, {searches_idx} / {len(searches)} completed, {len(export_jobs)} queued export jobs, {total_results} total bytes, {elapsed} elapsed")
        else:
            loggy.info(f"{len(running_jobs)} / {target_amt_jobs} jobs running, {len(paused_jobs)} jobs paused, {searches_idx} / {len(searches)} completed, {total_results} total bytes, {elapsed} elapsed")
        sleep(1)

    # second QUIT, instead of trying to finish export, just finalize the jobs
    if QUIT_C > 1:
        loggy.error(f"Ctrl+c pressed twice, finalizing {len(running_jobs)} jobs and quitting.")
...
def single_job_search(splunk_client: client.Service, options: SearchOptions) -> None:
    ...
    if options["singlejob_sid"]:
        loggy.info("Searching for job {}".format(options["singlejob_sid"]))
        ...
        NO_INTERRUPT=True
        if job:
            loggy.info(f"Found existing job {job.name}")
        else:
            loggy.error("Could not find job {}".format(options["singlejob_sid"]))
            quit()
    else:
        # create the search job
        job = create_search_job(splunk_client, options["search"].format(earliest=date_to_splunk(options["start_date"]), latest=date_to_splunk(options["end_date"])))
        loggy.info(f"Created job {job.name}")
    ...
    if not QUIT: # job is done
        loggy.info("Job completed, exporting to {}".format(options["export_options"]["merge_path"]))
        # export the job results and get count if any were exported
        result_count = export_job(job, options["export_options"])
        if result_count and result_count > 0:
            loggy.info("Exported {} results.".format(result_count))
        loggy.info("Finished!")
    else: # safe quit, pause job and save progress
        job.pause()
        loggy.info(f"Job {job.name} paused.")
...
@click.pass_context
@click.option("-u","--user",type=str,help="Splunk API User username",prompt=True,hide_input=False,required=True,envvar="SEARCHER_USER")
@click.option("--passwd",type=str,help="Splunk API User password",prompt=True,hide_input=True,required=True,envvar="SEARCHER_PASS")
@click.option("-h","--host",type=str,help="Splunk API Hostname or IP",prompt=True,required=True,envvar="SEARCHER_HOST")
@click.option("-p","--port",type=int,help="Splunk API port (usually 8089)",default=8089,envvar="SEARCHER_PORT")
@click.option("--debug", is_flag=True, help="Use debug level logging", default=False)
def cli(ctx: click.Context, user: str, passwd: str, host: str, port: int, debug: bool) -> None:
    """Main CLI group, sets up splunk client"""
    try:
        ctx.obj = client.connect(
                username=user, password=passwd, host=host, port=port
            )
    except ConnectionRefusedError as e:
        loggy.error(f"Connection refused to {host} on port {port}")
        quit()
    if debug:
...
@cli.command("resume")
@click.option("--progress-path", help="Path to load progress file from, defaults to loading newest", default="")
@click.option("--no-confirm", is_flag=True, help="Skip confirmation of chunk parameters", default=False)
@pass_splunk_client
def resume(splunk_client: client.Service, progress_path: str, no_confirm: bool) -> None:
    ...
    if progress_path == "":
        ...
        files = sorted([x for x in files if x.startswith("searcher_progress_")], reverse=True)
        if len(files) > 0:
            progress_path = abspath(files[0])
            loggy.info(f"Found progress file {progress_path}.")
        else:
            loggy.error(f"Could not find progress file, please pass it using the --progress-path option")
            quit()
    search_options = load_options(progress_path)
    validate_search_options(search_options)
    print_search_options(search_options)
    if search_options["chunk_options"]: # chunked search, rebuild searches array and confirm
...
@click.option("-d", "--export-path", type=str, default=join(getcwd(), "export"), envvar="SEARCHER_EXPORT_PATH", help="Path to export non-merged results to (./export/ by default)")
@click.option("--merge-path", type=str, help="File path to merge results into", default=None, envvar="SEARCHER_MERGE_PATH")
@click.option("-m", "--export-mode", help="Mode to use for results export", type=click.Choice(EXPORT_MODES), default="raw",envvar="SEARCHER_EXPORT_MODE")
@click.option("-s", "--chunk-size", type=float, help="Chunk size in decimal hours", default=0.5, envvar="SEARCHER_CHUNK_SIZE")
@click.option("-n", "--default-n-jobs",type=int,help="Default number of jobs to use when no limits are set",default=1, envvar="SEARCHER_DEFAULT_N_JOBS")
@click.option("--progress-path", help="Path to save progress file to in the event of ctrl+c", type=str, default=f"searcher_progress_{datetime.now().timestamp()}.pickle")
@click.option("--no-confirm", "--yes", is_flag=True, help="Skip confirmation of chunk parameters", default=False, envvar="SEARCHER_NO_CONFIRM")
@click.option("--limit", type=click.Tuple(types=[str,str,int]), required=False, multiple=True, help="Limit on amount of jobs by time of day. Format (HH:MM:SS or HH:MM:SS.FFFFFF): --limit START STOP N_JOBS [--limit START STOP N_JOBS [...]]", envvar="SEARCHER_LIMITS")
@click.option("--export-threads", type=int, default=None, help="Enable multi-threaded export and set number of threads", envvar="SEARCHER_EXPORT_THREADS")
def cli_chunk_search(
    ...
    eoptions = ExportOptions(export_path=abspath(export_path), export_mode=export_mode, merge_path=merge_path)
    search_options = SearchOptions(search=search, start_date=start, end_date=end, job_limits=job_limits, chunk_options=coptions, export_options=eoptions, progress_path=abspath(progress_path), singlejob_sid=None, export_threads=export_threads)
...
@click.argument("START", type=click.DateTime(), metavar="START-DATE")
@click.argument("END", type=click.DateTime(), metavar="END-DATE")
@click.option("-d", "--export-path", type=str, default=join(getcwd(), "export"), envvar="SEARCHER_EXPORT_PATH", help="Directory path to export non-merged results to (./export/ by default)")
@click.option("--merge-path", type=str, help="File path to merge results into, defaults to not merging if unset", default=None, envvar="SEARCHER_MERGE_PATH")
@click.option("-m", "--export-mode", help="Mode to use for results export", type=click.Choice(EXPORT_MODES), default="raw",envvar="SEARCHER_EXPORT_MODE")
@click.option("--no-confirm", "--yes", is_flag=True, help="Skip confirmation of chunk parameters", default=False, envvar="SEARCHER_NO_CONFIRM")
@click.option("--progress-path", help="Path to save progress file to in the event of ctrl+c", type=str, default=f"searcher_progress_{datetime.now().timestamp()}.pickle")
def cli_search(
    ...
    # build options state
    eoptions = ExportOptions(export_path=export_path, export_mode=export_mode, merge_path=merge_path)
    options = SearchOptions(export_options=eoptions, search=search, start_date=start, end_date=end, job_limits=None, chunk_options=None, progress_path=progress_path, singlejob_sid=None, export_threads=None)
